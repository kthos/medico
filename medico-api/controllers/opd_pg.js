  ...
  */
  ...
  for (let i = 0; i < tree.children.length; i++) {
    /*let sql = "SELECT  t.hn,date(t.visit_admit_date) dddd,year(t.visit_admit_date)+543 y \n" +
      ...
      WHERE t.hn = ? AND left(t.visit_admit_date,7) =? \n" +
      ...
      ,to_char(t.visit_admit_date,'DD') d  from medico_scan_doc t 
      WHERE t.hn = ? AND to_char(t.visit_admit_date,'YYYY-MM') =?
      GROUP BY  t.hn,dddd ORDER BY dddd DESC`; // ok
...
router.get('/type/:hn', async (req, res) => {
  ...
  for (let i = 0; i < tree.children.length; i++) {
    ...
    "ORDER BY vdate DESC";*/
    let sql = `SELECT t.hn,t.visit_admit_date vdate,t.department,t.doc_type 
    ,date_part('year',t.visit_admit_date) yy,date_part('year',t.visit_admit_date)+543 y
    ,to_char(t.visit_admit_date,'MM') m
    ,to_char(t.visit_admit_date,'DD') d ,t.file_name 
    FROM medico_scan_doc t LEFT JOIN medico_scan_code c ON c.code = t.doc_type
    WHERE t.hn = ? AND t.doc_type = ?
    ORDER BY vdate DESC`; // ok
