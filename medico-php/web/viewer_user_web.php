...
$user = empty($_GET['user']) ? 'null' : $_GET['user'];
if ($user == 'null') {
    exit;
}
$sql = "select count(his_user) as logged  from medico_scan_user_web where  is_allow = '1' and his_user='$user' limit 1";
$res = mysql_query($sql);
...
if (!empty($_GET['file'])) {
    $file = $_GET['file'];
    $sql = "insert into medico_scan_log (id,host,username,file_request,date_request,note1) values (UUID_SHORT(),'$ip','$user','$file',now(),'web')";
